...
class Ball:
    ...
    def action(self):              #0.1확률로 랜덤, 0.9확률로 greedy에서 최적의 경로를 가져오는 함수
        r = random.random()
        if r < self.epsilon:
    ...
    def greedyChoice(self):
        ...
        val_stop = self.keystate(2)

        if self.lookup(val_left) > self.lookup(val_right) \
                and self.lookup(val_left) > self.lookup(val_stop):
            return 0
        elif self.lookup(val_left) < self.lookup(val_right)\
                and self.lookup(val_right) > self.lookup(val_stop):
            return 1
        elif self. lookup(val_stop) > self.lookup(val_right)\
                and self.lookup(val_stop) > self.lookup(val_left):
            return 2
    ...
    def cyclestate(self):
        pos = self.canvas.coords(self.id)
        if pos[3] == PADDLE_HEIGHT:                   #패들의 윗부분[1]
            if self.y == 3:
                return START
            elif self.y == -3:
                return END
    ...
    def backup(self, newVal, idx):
        if idx >= 0 and self.learning:
            ...
            # 208,  303,  342,  3,  -3,  0,  1.02933 values에 있음
    ...
    def gameover(self):
        if self.learning:
            ...
            if self.is_paddle_hit():
                result_value = self.winval     #성공에 대한 보상치
                self.wincount += 1
            else:
    ...
    def writecsv(self):
        try:
            # Values 저장
            Fn = open("c:\\data\\pong_value.csv", 'w', newline='')
            ...
            # 성공/실패 횟수 저장
            Fn = open("C:\\data\\pong_score.csv", 'a', newline='')
            writer = csv.writer(Fn, delimiter=',')
    ...
    def loadcsv(self):
        try:
            Fn = open("c:\\data\\pong_value.csv", 'r')
...
if __name__ == '__main__':
    ...
    ball = Ball(canvas, paddle, 'red', announceterm=50, saveterm=5000)
    ...
    while 5000:
        ...
        if is_cycling:         #is_cycling가 true면 작동해라
            ball.action()      #ball.action에 패들이 움직이는 함수가 있음
